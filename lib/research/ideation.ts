...
function marketDefinition(query: string, target: string | null): { industry: string; companyProfile: string; decisionMaker: string } {
  const rules: Array<[RegExp, string, string, string]> = [
    ...
    [/commercial clean|\bcleaning (?:compan|business|service|team)|janitorial|proof.of.service/i, "commercial cleaning services", "commercial cleaning companies with mobile crews, recurring client sites, and contract proof-of-service obligations", "owner, field operations manager, or account operations director"],
